...
/* ============================================================================
   NUEVA FUNCIÓN: leer columna A (PAÍS) y B (ÁREA) desde 'Tags conceptos'
   y devolver valores únicos.
============================================================================= */
...
export async function appendFormResponse(formPayload) {
    ...
    const firmAreas = savedFirms.map(f => f.area || "").filter(Boolean).join(" · ");
    const firmCountries = savedFirms.map(f => f.country || "").filter(Boolean).join(" · ");
    ...
    const flat = {
        ...
        "Preferencia de cobro": formPayload.billingPreference || "",   // ← CORREGIDO
